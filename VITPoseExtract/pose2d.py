...
from VITPoseExtract.pose2d_utils import (
    read_img,
    box_convert_simple,
    bbox_from_detector,
    crop,
...
class Yolo(SimpleOnnxInference):
    def __init__(self, checkpoint, device='cuda', threshold_conf=0.05, threshold_multi_persons=0.04, input_resolution=(640, 640), threshold_iou=0.5, threshold_bbox_shape_ratio=0.4, cat_id=[1], select_type='max', strict=True, sorted_func=None, **kwargs):
            ...
            for i in range(results.shape[0]):
                ...
                if (bbox[-1] + 1 in cat_id) and (bbox[-2] > self.threshold_conf):
                    if self.select_type == 'max':
                        ...
                        bbox_size = (abs((bbox[2] + bbox[0]) / 2 - shape_raw[1]/2)) * -1
                    if i != max_idx and bbox_size > max_bbox_size * self.threshold_multi_persons and bbox_size < max_bbox_size:   # 添加后面的人
                        person_count += 1
...
class ViTPose(SimpleOnnxInference):
    def __init__(self, checkpoint, device='cuda', **kwargs):
        print(f"debug: Loading ViTPose model from {checkpoint}")
        ...
        **kwargs
        ...
        if self.detector is not None:
            ...
            for _image in images:
                ...
                det_result = self.detector(img[None], shape[None])[0]
                offset_list.append(offset)
                offset += len(det_result)
                cur_len_list.append(len(det_result))
                for result in det_result:
                    bboxes.append(result["bbox"])
        else:
            bboxes = [None] * len(images)

        bbox_list = [[] for i in range(len(images))]
        for i in range(len(images)):
            bbox_list[i] = bboxes[offset_list[i]:offset_list[i] + cur_len_list[i]]
