...
def get_all_keys(wds_root):
    ...
    key_set = set(random.sample(list(key_set), 500))
    return key_set
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_root', type=str, default='/workspace/ywh_data/pose_packed_wds_1102gymfinetune_step5_final',
                        help='Output root')

    args = parser.parse_args()

    input_root = '/workspace/ywh_data/pose_packed_wds_1024_step5_final'
    output_root = args.output_root
    os.makedirs(output_root, exist_ok=True)

    kept_subdirs = ['/workspace/ywh_data/pose_packed_wds_1024_step5/gymnastics_mb_v2025091601']
    kept_keys = list(get_all_keys(kept_subdirs[0]))
    breakpoint()
    ...
    for rank, wds_chunk in enumerate(chunks):
        p = Process(target=process_tar, args=(wds_chunk, output_root, rank, kept_keys))
        p.start()
        processes.append(p)
