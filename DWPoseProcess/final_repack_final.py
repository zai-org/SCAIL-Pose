        ...
        for data in data_iter:
            try:
                ...
                if len(motion_indices) > 81:
                    ...
                    obj['motion_indices'] = motion_indices[:81]
                    obj_list.append(obj)
                else:
                    obj = meta_dict.get(key, None)
                    obj_list.append(obj)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_root', type=str, default='/workspace/ywh_data/pose_packed_wds_1024_step5',
                        help='Input root')
    parser.add_argument('--output_root', type=str, default='/workspace/ywh_data/pose_packed_wds_1024_step5_final',
                        help='Output root')
    ...
    # 找到所有 .tar 文件
    subdirs = ['stunt_mb_v2025091601', 'gymnastics_mb_v2025091601', 'MMD', 'recall', 'motionarray_nowat']
    input_tar_paths = []
    for subdir in subdirs:
        new_paths = sorted(glob.glob(os.path.join(input_root, subdir, "**", "*.tar"), recursive=True))
        print(f"size of subdir: {subdir} is {len(new_paths)}")
        input_tar_paths.extend(new_paths)
