...
def process_one(subdir, video_name, test_mode, predictor, image_predictor):
    ...
    # 2) Driving → full-video mask for 1 person (replacement is always single-person)
    print(f"Getting driving masks from {mp4_path}...")
    drv_masks, drv_colors = get_mask_from_video(
        mp4_path, predictor, max_targets=1, sort_by='x', fixed_colors=None,
    )
    if len(drv_masks) == 0:
        raise RuntimeError(f"No valid persons detected in driving {mp4_path}")
    print(f"Driving person detected; color={drv_colors[0]}")
    ...
    ref_masks, ref_colors = get_mask_from_image(
        ref_image_path, image_predictor, max_targets=1, sort_by='x', fixed_colors=None,
    )
    if len(ref_masks) == 0:
...
if __name__ == '__main__':
    ...
    for i, subdir in enumerate(subdirs):
        ...
        print(f"\n{'='*60}")
        print(f"[{i+1}/{len(subdirs)}] {subdir}  (video_name={args.video_name}, test_mode={args.test_mode})")
        print(f"{'='*60}")
        t0 = time.time()
        try:
            process_one(subdir, args.video_name, args.test_mode,
